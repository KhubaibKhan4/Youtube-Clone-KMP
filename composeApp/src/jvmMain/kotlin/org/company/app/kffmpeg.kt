...
class KAVFormatContext {
    val fmtCtx = avformat.AVFormatContext(null)
    ...
    fun findMetadata(): Map<String, String> {
        ...
        do {
           // entry = avutil.av_dict_iterate(fmtCtx.metadata(), entry)
            if (entry != null) {
                val key = entry.key().getString(Charset.defaultCharset().toString())
                val value = entry.value().getString(Charset.defaultCharset().toString())
    ...
    fun findVideoStreams(): List<KVideoStream> =
        (0 until fmtCtx.nb_streams()).mapNotNull { i ->
            ...
            if (type == KAVMediaType.VIDEO) {
                ...
                val codecName = avcodec.avcodec_get_name(stream.codecpar().codec_id()).getString(
    ...
    fun findCodec(videoStream: KVideoStream): KAVCodec {
        ...
        val name = avcodec.avcodec_get_name(codecId).getString(Charset.defaultCharset().toString())
    ...
    private fun findHWDecoder(codecId: Int): List<KHWDecoder> {
        ...
        var index = 0
        do {
            val config: avcodec.AVCodecHWConfig? = avcodec.avcodec_get_hw_config(codec, index)
        ...
        while (hwType != avutil.AV_HWDEVICE_TYPE_NONE && index++ < 20) {
            val name = avutil.av_hwdevice_get_type_name(hwType).getString(Charset.defaultCharset().toString())
...
class KAVCodecContext(codecParams: avcodec.AVCodecParameters, hwDecoder: KHWDecoder? = null) {
    val codecCtx: avcodec.AVCodecContext = avcodec.avcodec_alloc_context3(null)
    ...
    init {
        ...
        if (hwDecoder != null) {
            ...
            // HWAccel
            val hwDeviceCtx = avutil.AVBufferRef()
            errCheck(
                avutil.av_hwdevice_ctx_create(hwDeviceCtx, hwDecoder.type, null as BytePointer?, null, 0),
                "Unable to create hw decoder ${hwDecoder.type}:${hwDecoder.name} for ${codec.name().getString(
                    Charset.defaultCharset().toString()
                )}"
            )
...
class KFrameGrabber(
    ...
    private val pkt = avcodec.AVPacket()
